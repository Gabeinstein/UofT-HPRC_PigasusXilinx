...
assign pdumeta_cnt = pdumeta_cpu_csr_readdata[9:0];
    ...
    integer cnt=0;
    integer in_pkt_push = 0;
    integer out_pkt_pop = 0;
    integer in_usr_push = 0;
    integer out_usr_pop = 0;
    integer in_meta_push = 0;
    integer out_meta_pop = 0;

    always@(posedge clk) begin
        cnt<=cnt+1;
        if (my_dm2sm.port[0].valid && my_dm2sm.port[0].ready) begin
            in_pkt_push <= in_pkt_push + 1;
	        $display("clk %d in pkt push %x", cnt, my_dm2sm.port[0].data);
        end
        if (my_dm2sm.port[2].valid && my_dm2sm.port[2].ready) begin
	        $display("clk %d out pkt pop %x", cnt, my_dm2sm.port[2].data);
	        out_pkt_pop <= out_pkt_pop + 1;
        end
        if (my_dm2sm.port[1].valid && my_dm2sm.port[1].ready) begin
	        $display("clk %d in usr push %x", cnt, my_dm2sm.port[1].data);
	        in_usr_push <= in_usr_push + 1;
        end
        if (my_dm2sm.port[3].valid && my_dm2sm.port[3].ready) begin
	        $display("clk %d out usr pop %x", cnt, my_dm2sm.port[3].data);
	        out_usr_pop <= out_usr_pop + 1;
        end
        if (my_dm2sm.port_meta[0].valid && my_dm2sm.port_meta[0].ready) begin
	        $display("clk %d in meta push %x", cnt, my_dm2sm.port_meta[0].data);
	        in_meta_push <= in_meta_push + 1;
        end
        if (my_dm2sm.port_meta[1].valid && my_dm2sm.port_meta[1].ready) begin
	        $display("clk %d out meta pop %x", cnt, my_dm2sm.port_meta[1].data);
	        out_meta_pop <= out_meta_pop + 1;
        end
     end
    ...
    /*unified_pkt_fifo_rosebud#(.FIFO_NAME("[top] fifo0"), .MEM_TYPE("M20K"), .DUAL_CLOCK(0), .USE_ALMOST_FULL(1), .FULL_LEVEL(450), .SYMBOLS_PER_BEAT(64), .BITS_PER_SYMBOL(8), .FIFO_DEPTH(512)) my_fifo0_rosebud (
        .in_clk(clk),
        .in_reset(rst_n),
        .fill_level(dm_nopayload_pkt_csr_readdata),
        .in_data(fifo0_in_direct.txP.),
        .out_data(ethernet_out0_direct.txP)
    );*/
